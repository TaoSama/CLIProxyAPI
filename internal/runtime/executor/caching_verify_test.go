...
func TestEnsureCacheControlTTL(t *testing.T) {
	...
	t.Run("1h injects ttl on every breakpoint", func(t *testing.T) {
		out := upgradeClaudeCacheControlTTL(ensureCacheControl(input), "1h")
		for _, p := range []string{"system.0.cache_control", "messages.2.content.0.cache_control"} {
	...
	t.Run("1h survives normalizeCacheControlTTL (all-1h is ordering-safe)", func(t *testing.T) {
		out := normalizeCacheControlTTL(upgradeClaudeCacheControlTTL(ensureCacheControl(input), "1h"))
		for _, p := range []string{"system.0.cache_control.ttl", "messages.2.content.0.cache_control.ttl"} {
