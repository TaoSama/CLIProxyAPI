...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	// Determine the ephemeral cache TTL. Per-credential cloak.cache-ttl takes
	// precedence; Claude Code CLI fingerprint profiles default to the 1h pool to
	// match native's extended-cache-ttl-2025-04-11 behavior.
	cacheTTL := resolveInjectedCacheTTL(e.cfg, auth)
	if cacheTTL == "" && fp.ProfileClaudeCodeCLI {
		cacheTTL = claudeCacheControlTTL1h
	}
	if cpaOwnsCacheControl && cacheTTL != "" {
		body = upgradeClaudeCacheControlTTL(body, cacheTTL)
	}
