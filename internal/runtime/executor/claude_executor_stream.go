...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	// Per-credential cloak.cache-ttl takes precedence. Native Claude Code
	// subagents default to 5m unless 1h is explicitly configured; probes omit
	// both 1h cache and extended-cache-ttl.
	...
	if isProbeOrHelper || (isSubagent && !subagent1h && cacheTTL == "") {
		...
		} else if fp.ProfileClaudeCodeCLI && (!isSubagent || subagent1h) {
