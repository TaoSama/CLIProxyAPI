...
func convertOpenAIResponsesRequestToClaude(modelName string, inputRawJSON []byte, stream, preserveEmptyThinkingBlocks bool) []byte {
	...
	// Collect all tool outputs per call_id so matched incremental outputs
	// can become one Claude tool_result while orphan outputs still follow the
	// upstream repair path as plain user text.
	...
	for _, item := range inputItems {
		...
		case "function_call_output", "custom_tool_call_output":
			// Map matched outputs to user tool_result. Multiple outputs for the
			// same call_id are merged into one tool_result emitted at the first
			// occurrence; orphan outputs remain plain user text.
			rawID := common.ExtractResponsesCallID(item)
			if rawID != "" {
			...
			if _, paired := emittedRawToolUses[rawID]; rawID == "" || !paired {
				...
				if outputs := toolOutputsByID[rawID]; rawID != "" && len(outputs) > 0 {
					output = outputs[len(outputs)-1].Get("output")
				}
			...
			var toolResult []byte
			outputs := toolOutputsByID[rawID]
			if len(outputs) == 0 {
				outputs = []gjson.Result{item}
			}
			for i, outputItem := range outputs {
				...
				part = applyResponsesToolResultContent(part, outputItem.Get("output"))
				if i == 0 {
					toolResult = part
				} else {
					toolResult = mergeResponsesToolResults(toolResult, part)
				}
			}
