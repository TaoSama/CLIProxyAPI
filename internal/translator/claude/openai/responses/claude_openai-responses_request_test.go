...
func TestConvertOpenAIResponsesRequestToClaude_DeduplicatesToolOutputs(t *testing.T) {
	...
	// toolu_dup has two outputs that are merged into one tool_result.
	mergedContent := messages[2].Get("content.0.content").Array()
	if len(mergedContent) != 2 {
		t.Fatalf("toolu_dup merged content count = %d, want 2; output=%s", len(mergedContent), string(out))
	}
	if got := mergedContent[0].Get("text").String(); got != "first result" {
		t.Fatalf("toolu_dup content[0].text = %q, want 'first result'", got)
	}
	if got := mergedContent[1].Get("text").String(); got != "final result" {
		t.Fatalf("toolu_dup content[1].text = %q, want 'final result'", got)
	}
	...
	// call.custom:dup has two outputs that are merged into one tool_result.
	customMerged := msg4Blocks[0].Get("content").Array()
	if len(customMerged) != 2 {
		t.Fatalf("call_custom_dup merged content count = %d, want 2; output=%s", len(customMerged), string(out))
	}
	if got := customMerged[0].Get("text").String(); got != "custom first" {
		t.Fatalf("call_custom_dup content[0].text = %q, want 'custom first'", got)
	}
	if got := customMerged[1].Get("text").String(); got != "custom final" {
		t.Fatalf("call_custom_dup content[1].text = %q, want 'custom final'", got)
	}
